    ...
    const TeamState = {
        ...
        async loadTimesheetsData() {
            try {
                ...
                console.log(`[TIMESHEETS-FILTER-DEBUG] loadTimesheetsData called at ${timestamp}`, this.filters.timesheets);
                ...
                console.log(`[TIMESHEETS-FILTER-DEBUG] API params:`, params);
        ...
        setupEventListeners() {
            ...
            $(document).on('change', '#tt-filter-start, #tt-filter-end, #tt-filter-employee, #tt-filter-status', () => {
                const timestamp = new Date().toISOString();
                console.log(`[TIMESHEETS-FILTER-DEBUG] Filter change at ${timestamp}`, {
                    startDate: $('#tt-filter-start').val(),
                    endDate: $('#tt-filter-end').val(),
                    employee: $('#tt-filter-employee').val(),
                    status: $('#tt-filter-status').val()
                });
                ...
                console.log(`[TIMESHEETS-FILTER-DEBUG] Updated filters object:`, this.filters.timesheets);
        ...
        renderTimesheetsTable() {
            const timestamp = new Date().toISOString();
            console.log(`[TIMESHEETS-FILTER-DEBUG] renderTimesheetsTable called at ${timestamp}`, this.filters.timesheets);
                ...
                <div class="tt-timesheets-content">
                    ...
                    <div class="tt-filters-bar">
                        <div class="tt-filter-group">
                            ...
                            <input type="date" class="tt-filter-input" id="tt-filter-start" value="${this.filters.timesheets.startDate || this.getWeekStart()}">
                            <span class="tt-time-separator">to</span>
                            <input type="date" class="tt-filter-input" id="tt-filter-end" value="${this.filters.timesheets.endDate || this.getWeekEnd()}">
                        </div>
                        <div class="tt-filter-group">
                            <label class="tt-filter-label">Employee:</label>
                            <select class="tt-filter-select" id="tt-filter-employee">
                                <option value="">All Employees</option>
                                ${(this.employees || []).map(e => `<option value="${e.id}" ${this.filters.timesheets.employee == e.id ? 'selected' : ''}>${e.first_name} ${e.last_name}</option>`).join('')}
                            </select>
                        </div>
                        <div class="tt-filter-group">
                            <label class="tt-filter-label">Status:</label>
                            <select class="tt-filter-select" id="tt-filter-status">
                                ...
                                <option value="pending" ${this.filters.timesheets.status === 'pending' ? 'selected' : ''}>Pending</option>
                                <option value="approved" ${this.filters.timesheets.status === 'approved' ? 'selected' : ''}>Approved</option>
                                <option value="rejected" ${this.filters.timesheets.status === 'rejected' ? 'selected' : ''}>Rejected</option>
                            </select>
                        </div>
                        <div class="tt-filter-actions">
