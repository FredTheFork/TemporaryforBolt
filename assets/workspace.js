...
jQuery(($) => {
  ...
  function jobCardHTML(job, stage) {
    ...
    if (endDateRaw) {
      const dueDate = new Date(endDateRaw);
      const now = new Date();
      const diffTime = dueDate - now;
      const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
      
      if (diffDays < 0) dueDateInfo = { text: `${Math.abs(diffDays)}d overdue`, overdue: true };
      else if (diffDays === 0) dueDateInfo = { text: 'Due today', overdue: false };
      else if (diffDays === 1) dueDateInfo = { text: 'Due tomorrow', overdue: false };
      else if (diffDays <= 7) dueDateInfo = { text: `Due in ${diffDays}d`, overdue: false };
      else dueDateInfo = { text: endDate, overdue: false };
    }

    // Build meta tags to match lead card structure
    let metaTags = '';
    
    // Progress (mapped to value slot)
    if (progress > 0) {
      metaTags += `
        <span class="pi-meta-tag value">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2v20M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
    ...
    // Expenses (mapped to invoice slot)
    if (hasExpenses) {
      const expenseAmount = formatCurrency(job.total_expenses) || '£0';
      metaTags += `
        <span class="pi-meta-tag invoice" title="Expenses incurred">
          <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
          ${expenseAmount}
        </span>
      `;
    ...
    return `
      <div class="${cardClasses}" data-id="${job.id}" data-stage="${stage}" data-end-date="${escapeHtml(endDateRaw)}">
        <div class="pi-card-actions">
          ...
          <button class="pi-btn pi-btn-icon pi-remove" title="Delete job"></button>
        </div>
        <div class="pi-card-header">
